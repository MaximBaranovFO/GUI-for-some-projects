 ...
 */
public class ZPIThDirListWorkRead implements Runnable{
    //private ThreadLocal<ThDirListRule> ruleDirListReadWork;
    private ZPIThDirListRule ruleDirListReadWork;
    
    ZPIThDirListWorkRead(final ZPIThDirListRule outerRuleReader){
    ...
    @Override
    public void run(){
        System.out.println(ZPIThDirListWorkRead.class.getCanonicalName() + " run and say " + this.ruleDirListReadWork.toString());
        this.ruleDirListReadWork.setTrueRunnedDirListWorkReader();
        ThreadLocal<ZPIThDirListLogicRead> logicReader = new ThreadLocal<ZPIThDirListLogicRead>();
        try{
            logicReader.set(new ZPIThDirListLogicRead());
            /**
